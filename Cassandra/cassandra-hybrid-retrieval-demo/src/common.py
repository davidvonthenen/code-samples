"""Cassandra and embedding helpers for the hybrid retrieval demo."""
...
def ensure_schema(session: Session) -> None:
    ...
    # One SAI index per lane the demo runs: keywords for lexical grounding,
    # category / model / model_year for metadata filters, embedding for ANN.
    session.execute(
        f"""
        CREATE INDEX IF NOT EXISTS docs_keywords_sai
        ON {TABLE} (keywords)
        USING 'sai'
        """
    )
    session.execute(
        f"""
        CREATE INDEX IF NOT EXISTS docs_category_sai
        ON {TABLE} (category)
        USING 'sai'
    ...
    session.execute(
        f"""
        CREATE INDEX IF NOT EXISTS docs_model_sai
        ON {TABLE} (model)
        USING 'sai'
        """
    )
    session.execute(
        f"""
        CREATE INDEX IF NOT EXISTS docs_model_year_sai
        ON {TABLE} (model_year)
        USING 'sai'
        """
    )
