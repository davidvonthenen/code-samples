# Apache Cassandra Code Samples

## Description

The following folders contain code samples related to [Apache Cassandra](https://cassandra.apache.org/) on NetApp Instaclustr. These samples cover a range of Cassandra use cases including client connectivity, data modeling, and application patterns.

## Projects

| Project | Description |
|---------|-------------|
| [cassandra-vector-demo](./cassandra-vector-demo/) | Local Cassandra 5 ANN retrieval demo (JVector, no LLM) |
| [cassandra-hybrid-retrieval-demo](./cassandra-hybrid-retrieval-demo/) | Local Cassandra 5 hybrid retrieval demo: SAI keywords, ANN, and filtered ANN on one table |

## Getting Started

Each sample has its own README that describes the sample, any prerequisites, examples, and anything else relevant to the code.

### Prerequisites

- Apache Cassandra cluster (e.g., [Instaclustr managed Apache Cassandra](https://www.instaclustr.com/platform/managed-apache-cassandra/))
- Additional prerequisites vary per project — see individual READMEs

## Additional Materials

Each README will also include a link to a blog post, YouTube video, or conference presentation to provide more context and additional information.

## Authors

* **Mariah McLaughlin** - [NetApp Instaclustr](https://github.com/Instaclustr)

See also the list of [MAINTAINERS](https://github.com/instaclustr/code-samples/blob/main/Maintainer.md) who participated in projects in this repository.

## License

See individual project READMEs for license details.

### About

If you find an issue with the code sample, please feel free to create an issue and a [MAINTAINER](https://github.com/instaclustr/code-samples/blob/main/Maintainer.md) will address it.
