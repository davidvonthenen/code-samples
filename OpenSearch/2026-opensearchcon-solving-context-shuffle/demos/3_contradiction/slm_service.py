...
# Hardcoded value to enable or disable the reasoning capabilities of the LLM
...
def _load_local_gguf_llm(settings: Settings) -> Any:
    ...
    # Initialize without a fixed chat_format to allow dynamic template injection
    llm = Llama(**kwargs)

    # Attempt to extract the native Jinja template and inject the thinking toggle
    # Using a string key here ensures we actually retrieve the metadata!
    chat_template = llm.metadata.get("tokenizer.chat_template")
    if chat_template:
        try:
            from llama_cpp.llama_chat_format import Jinja2ChatFormatter
            
            injected_template = chat_template
            if ENABLE_THINKING:
                print("***** THINKING IS ENABLED *****")
                print("***** THINKING IS ENABLED *****")
                print("***** THINKING IS ENABLED *****")
            else:
                flag_str = "true" if ENABLE_THINKING else "false"
                injected_template = f"{{%- set enable_thinking = {flag_str} %}}\n{{%- set reasoning_budget_tokens = 0 %}}\n" + chat_template
                
            llm.chat_handler = Jinja2ChatFormatter(template=injected_template)
            llm.chat_format = None
            LOGGER.info("Injected ENABLE_THINKING=%s into Jinja template.", ENABLE_THINKING)
        except ImportError:
            LOGGER.warning("Jinja2ChatFormatter unavailable. Falling back to chatml.")
            llm.chat_format = "chatml"
        except Exception as exc:
            LOGGER.warning("Failed to apply thinking flag: %s", exc)
            llm.chat_format = "chatml"
    else:
        llm.chat_format = "chatml"
...
def _load_local_mlx_llm(settings: Settings) -> Tuple[Any, Any]:
    ...
    # Intercept and modify the native Jinja template string
    if hasattr(tokenizer, "chat_template") and isinstance(tokenizer.chat_template, str):
        if ENABLE_THINKING:
            ...
            flag_str = "true" if ENABLE_THINKING else "false"
            tokenizer.chat_template = f"{{%- set enable_thinking = {flag_str} %}}\n{{%- set reasoning_budget_tokens = 0 %}}\n" + tokenizer.chat_template
            LOGGER.info("Injected ENABLE_THINKING=%s into MLX tokenizer template.", ENABLE_THINKING)
